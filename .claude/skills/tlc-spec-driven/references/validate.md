...
For each user story across `spec/NN-*.md`:
...
From the edge cases in each spec slice:
...
Update requirement statuses in `spec/INDEX.md` (and the matching spec slice):
...
- **Update traceability** — Every verified requirement updates its status in `spec/INDEX.md` and its spec slice
