...
2. **Spec-level validation (independent Verifier sub-agent, always-on, never prompted):** After all tasks of a **single spec** are done, validation for that spec runs automatically - the orchestrator dispatches a **fresh Verifier sub-agent** (see [sub-agents.md](sub-agents.md)). One spec, one Verifier run, one report; a feature with four specs gets four. Do NOT ask the user whether to run it; it is the safety net, not an opt-in. User interaction is limited to interactive UAT (for user-facing features) and acting on a FAIL verdict ("fix these gaps now?"). The Verifier:
   ...
   - Scopes coverage to **this spec's git diff surface** - the commit range covering its tasks, not the full feature and not the repository
   ...
   - Writes `.specs/features/[feature]/validation/NN-[slug].md` with the full evidence report
...
Go through tasks.md, filtered to the tasks whose `Spec:` field names the spec under verification:

- [ ] All of this spec's tasks marked done?
- [ ] Any blocked or partial?
- [ ] Any task from another spec accidentally in scope? (if so, the phase partition is broken - report it)

### 2. Spec-Anchored Acceptance Criteria Check

For each acceptance criterion in the spec file under verification (`specs/NN-[slug].md`), the Verifier re-derives the **spec-defined expected outcome** and confirms the test's actual assertion matches it:
...
From this spec's Edge Cases section:
...
3. **Inject a behavior-level fault** into the scratch copy of the new code introduced by this spec. Choose a mutation proportional to the code's risk:
...
| ------- | ------------ |
| Default (all specs) | Lightweight fault-injection: 1-3 targeted behavior-level mutations per spec, focused on the highest-risk new code |
...
1. **Write the persisted report** to `.specs/features/[feature]/validation/NN-[slug].md` (see template below). This file is the evidence artifact for THIS spec - it survives the session and can be referenced by CI, reviewers, or future agents.
...
**Deterministic backing (run it, do not eyeball it).** After writing the report, run `python3 <skill-dir>/scripts/validate_state.py <feature>`. It confirms every spec's report is real - present at `validation/<spec-id>.md`, verdict filled to PASS, and backed by at least one `file:line` evidence citation - so a missing, hollow, placeholder, or FAIL report cannot slip through as done. While earlier specs are still in flight it only gates the specs whose tasks are all complete. A non-zero exit means the feature is NOT done: repair the report or route the FAIL gaps to fix tasks, then re-run. This is the closing gate of Execute and runs automatically, the same way the lessons layer runs at distillation; it is never a manual step. If no code-execution tool is available, confirm the same by reading the report.

### 10. Distill Lessons (MANDATORY when the validation report has signal)
...
## Validation Report Template (`.specs/features/[feature]/validation/NN-[slug].md`)

```markdown
# [Feature] / `NN-[slug]` Validation

**Date**: [YYYY-MM-DD]
**Spec**: `.specs/features/[feature]/specs/NN-[slug].md`
**Diff range**: [commit range covering this spec's tasks]
...
Update the requirement statuses in `specs/NN-[slug].md`, and the spec's row in `overview.md`'s Spec Index:

| Requirement | Previous Status | New Status   |
| ----------- | --------------- | ------------ |
| [PREFIX]-01 | Implementing    | ✅ Verified  |
| [PREFIX]-02 | Implementing    | ❌ Needs Fix |
...
- **Validation is never prompted** - it always runs after the last task of every spec; do not ask the user whether to run it
- **One spec, one report** - the Verifier reads a single spec file and scopes its diff to that spec's commits; the feature is done only when every spec has a PASS
...
- **Update traceability** - Every verified requirement updates its spec file's status, and the Spec Index row in `overview.md`
- **Always write the report file** - `.specs/features/[feature]/validation/NN-[slug].md` is the persisted evidence artifact
