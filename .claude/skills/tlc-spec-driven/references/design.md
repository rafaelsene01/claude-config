...
Read `.specs/features/[feature]/spec/INDEX.md` first, then each `spec/NN-*.md` one at a time before designing. If `.specs/features/[feature]/context.md` exists, load it too — it contains implementation decisions that constrain the design (layout choices, behavior preferences, interaction patterns). Decisions marked as "Agent's Discretion" are yours to decide.
...
**Specs**: `.specs/features/[feature]/spec/` (see `INDEX.md` for the spec map)
