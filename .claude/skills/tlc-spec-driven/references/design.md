...
Read `.specs/features/[feature]/overview.md` (problem, Spec Index, dependency graph) before designing, then read the individual `specs/NN-[slug].md` files **one at a time** as you design the components each one needs - never load the whole `specs/` directory at once. Legacy features with a flat `spec.md` read that instead. If `.specs/features/[feature]/context.md` exists, load it too - it contains implementation decisions that constrain the design (layout choices, behavior preferences, interaction patterns). Decisions marked as "Agent's Discretion" are yours to decide.
...
Each component: Purpose, Location, Interfaces, Dependencies, What it reuses, and **which spec(s) it serves**. A component serving three specs is a shared dependency - check that the spec dependency graph in `overview.md` accounts for it, or that it is owned by a scaffolding spec the others depend on.
...
**Specs covered**: `NN-[slug]`, `NN-[slug]`, `NN-[slug]`
