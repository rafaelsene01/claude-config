...
Coverage Matrix and Gate Check Commands from tasks.md, **the single spec file its batch belongs to**
(`.specs/features/[feature]/specs/NN-[slug].md` - never the whole `specs/` directory), and the
relevant design context. A batch is one or more consecutive whole phases of ONE spec, packed to ~7
tasks. The worker executes ALL tasks in its
...
2. Tests MUST be derived from the task's "Done when" criteria and the ACs of **the spec named in the task's `Spec:` field** (`.specs/features/[feature]/specs/NN-[slug].md`) - **not** from the implementation. Each test encodes what the spec requires; never write tests by reading the code and asserting what it currently does.
...
4. Edge cases listed in that spec that apply to this task get test cases too. Do NOT pull ACs from a sibling spec - if a criterion you need lives in another spec, the task is in the wrong phase.
...
4. **Test Adequacy Review (MANDATORY - hard gate).**
   ...
   **Check B - Non-shallow litmus.** Reject each of the following shallow patterns:
   ...
   - Happy-path only when the task's "Done when" or its spec lists edge cases
   ...
   *Honest caveat:* This is an inspection-based review (model judgment), complementary to - not a replacement for - the deterministic gate. The gate confirms the test suite runs; the spec-level discrimination sensor (step 9) confirms the tests can detect regressions. This review confirms the suite is meaningful and bounded.
...
1. Mark the task complete in `tasks.md`. Update the requirement traceability table in the task's spec file (`specs/NN-[slug].md`) if requirement IDs are used.
...
- Include only files listed in the task - plus the `tasks.md` / spec-file status updates for this task
...
### 9. Spec-Level Validation (after the LAST task OF EACH SPEC - MANDATORY, always runs)

When the task you just completed is the **last task of a spec** (every task whose `Spec:` field names it is marked complete), you MUST run validation for that spec before starting the next one. **This is not optional and is never prompted - it runs automatically.** Do not stop at the final task's commit, and do not batch validation until the end of the feature: a spec is a deliverable, and it closes with its own verdict.
...
- Spec-level validation (step 9): one trustworthy independent gate per spec, always-on, Verifier sub-agent does it.
- Feature completion: the feature is done when **every** spec has a PASS report - confirmed by `python3 <skill-dir>/scripts/validate_state.py <feature>`.
...
- The spec file `.specs/features/[feature]/specs/NN-[slug].md` (its ACs = source of truth) - that one file only
- The git diff surface for **this spec's** commits (the commit range covering its tasks)
- The test files in scope
- `validate.md` as its operating checklist

**What the Verifier does** (full procedure in [sub-agents.md](sub-agents.md); operating checklist in [validate.md](validate.md)): a spec-anchored coverage check (evidence-or-zero, each asserted value matched to the spec outcome) plus a discrimination sensor (behavior-level mutations run in a scratch state and then discarded), after which it writes `.specs/features/[feature]/validation/NN-[slug].md` (PASS/FAIL, per-AC evidence, sensor result, diff range) and returns a compact verdict + ranked gaps in chat. It runs read-only over the real tree and does NOT fix.

If the Verifier returns FAIL, the orchestrator routes the ranked gaps back to an implementer as fix tasks **for that spec**, then re-dispatches the Verifier - bounded to **3 fix→re-verify iterations** before escalating to the user. A spec that has not passed does not unblock the specs that depend on it.

If you are unsure whether more tasks remain for the current spec, check `tasks.md`: if every task carrying that `Spec:` value is marked complete, dispatch the Verifier now.
...
**Spec**: `NN-[slug]`
**Reading**: task definition from tasks.md + `specs/NN-[slug].md`
...
**After the LAST task of each spec:** dispatch the Verifier sub-agent (see step 9 and [sub-agents.md](sub-agents.md)) for independent spec-level validation, including the spec-anchored check and discrimination sensor. Validation always runs automatically - never prompted. A spec is not done until its Verifier reports PASS and `validation/NN-[slug].md` is written; Execute is not done until **every** spec has one, confirmed deterministically by `python3 <skill-dir>/scripts/validate_state.py <feature>` (exit non-zero = not done); see [validate.md](validate.md).
...
- **Don't stop at the last commit** - Spec-level validation (step 9) closes every spec, not just the last one
- **One spec at a time** - read the spec file named in the task's `Spec:` field; never pull criteria from a sibling spec
