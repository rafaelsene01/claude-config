...
Read the **Spec Map** in tasks.md first — it is the controller. Pick the next wave: every spec whose `Depends on` specs are `Done`. Within the wave, take that spec's tasks.

User specifies ("implement T2.1" or "implement S2") or suggest the next available wave.

If there is no tasks.md, use the inline execution plan above.

### 2. Verify Dependencies

Check two levels:

- **Spec level** — the Spec Map's `Depends on` column. A spec whose dependency is not `Done` cannot start.
- **Task level** — the task's `Depends on` field.

Load ONLY the spec slice this task belongs to (`spec/NN-*.md`, from the task's `Spec:` field). Never load sibling specs.

❌ If blocked: "T2.1 belongs to S2, which depends on S1 (still In Progress). Should I finish S1 first?"
...
- Edge cases from the task's spec slice (`spec/NN-*.md`) that apply to this task get test cases too
...
Mark the task complete in tasks.md and update the Spec Map row when the last task of a spec finishes. Update requirement traceability in `spec/INDEX.md` and the spec slice if requirement IDs are used.
