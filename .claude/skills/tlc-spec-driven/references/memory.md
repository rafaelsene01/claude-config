...
- **Spec**: [e.g., `02-idempotencia` - the spec currently being executed]
- **Specs verified**: [spec IDs with a PASS report, or "none"]
...
4. **Reconcile with git before editing anything:**
   ...
   - Which specs already have a report under `validation/` and whether each is PASS (`python3 <skill-dir>/scripts/validate_state.py <feature>`) - a spec with a PASS report is closed; resume at the next spec, not inside it
