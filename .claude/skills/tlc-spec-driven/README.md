...
**TLC Spec-Driven** transforms how AI agents approach software projects. Instead of a rigid, bureaucratic pipeline, it uses **5 adaptive phases** that auto-size based on complexity — applying full rigor for complex features and skipping ceremony for simple ones:
...
**Explore** interviews you in rounds until nothing is silently assumed, then **Specify** splits
the demand into several self-contained spec files — one per cohesive capability — and **Tasks**
becomes the controller that tracks which specs ran and which can run in parallel.

**The complexity is in the system, not in your workflow.** You talk naturally — the skill decides how deep to go:

| Scope                               | What happens                                                      |
| ----------------------------------- | ----------------------------------------------------------------- |
| **Small** (≤3 files)                | Quick mode — describe → implement → verify → commit (no Explore)  |
| **Medium** (clear feature)          | Explore (1-2 rounds) → 1-3 specs → Execute (design inline)        |
| **Large** (multi-component)         | Explore until frontier empty → 3-8 specs → design → tasks → waves |
...
│       │   ├── 01-[capability].md    # Self-contained slice with traceable IDs (AUTH-01...)
│       │   ├── 02-[capability].md
│       │   └── 03-[capability].md
│       ├── context.md  # User decisions for gray areas (only when needed)
│       ├── design.md   # Architecture and components (only for large/complex)
│       └── tasks.md    # Spec Map (status + parallel waves) + atomic tasks per spec
│
...
| Trigger Pattern                           | Description                             |
| ----------------------------------------- | --------------------------------------- |
| `Explore demand`, `Grill me`, or any feature request | Rounds of questions until the frontier is empty |
| `Specify feature`, `Define requirements`  | Create spec/ with one file per capability + INDEX.md |
...
| `Break into tasks`, `Create tasks`        | Create tasks.md: Spec Map + atomic breakdown |
...
Agent: ❓ Q1 - Session lifetime: how long should a session stay valid?
       ➡️ 15-minute access token + 7-day refresh token
       ❓ Q2 - ... (asks the whole frontier, one round at a time)
       [Dispatches sub-agents to find what the codebase already has]
       [Frontier empty → presents Understanding Summary → you confirm]

       ✅ Created spec/INDEX.md + 4 spec files:
          01-credential-auth, 02-token-store, 03-route-guard, 04-password-reset
...
Agent: [Creates tasks.md: Spec Map with 4 specs in 3 waves + 8 atomic tasks]
...
A: Each requirement gets a unique ID (e.g., `AUTH-01`) in its spec slice, rolled up in `spec/INDEX.md`. Tasks reference these IDs, and validation checks which requirements are verified. You get a clear trail from spec → design → task → commit.
