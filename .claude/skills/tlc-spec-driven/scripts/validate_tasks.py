...
Additionally, when the feature uses the multi-spec layout (a sibling specs/
directory), the spec partition is enforced:
  ERROR  - a task is missing its `Spec` field
  ERROR  - a task's `Spec` names no file under specs/
  ERROR  - a phase contains tasks from more than one spec
  ERROR  - a spec under specs/ has no task at all
  ERROR  - a task depends on a task of another spec that the spec dependency
           graph (each spec's `Depends on`) does not declare
...
REQUIRED_SECTIONS_MULTI = ["Spec Partition"]
SPEC_ID_RE = re.compile(r"\d{2}-[a-z0-9][a-z0-9-]*")
...
def parse_tasks(lines):
    ...
    for ln in lines:
        ...
        if m:
            ...
            tasks[current] = {"deps": set(), "tests": None, "gate": None, "where": "", "spec": None}
            continue
        if current is None:
            continue
        stripped = ln.strip()
        ...
        sm = re.match(r"^\*{0,2}Spec\*{0,2}\s*:\s*(.*)$", stripped, re.IGNORECASE)
        if sm:
            found = SPEC_ID_RE.search(sm.group(1))
            tasks[current]["spec"] = found.group(0) if found else sm.group(1).strip().strip("`")
    return tasks


def load_spec_graph(feature_dir):
    """Return (spec_ids:set, deps:{spec_id:set}) from <feature_dir>/specs/*.md.

    Returns (None, None) when the feature does not use the multi-spec layout.
    """
    ...
    for fname in sorted(os.listdir(specs_dir)):
        ...
        ids.add(sid)
        deps[sid] = set()
        with open(os.path.join(specs_dir, fname), encoding="utf-8", errors="replace") as f:
            head = f.read().splitlines()[:40]
        for ln in head:
            m = re.match(r"^\*{0,2}Depends on\*{0,2}\s*:\s*(.*)$", ln.strip(), re.IGNORECASE)
            if m:
                body = m.group(1)
                if not re.match(r"^\s*(none|-|n/?a)\s*$", body.strip(), re.IGNORECASE):
                    deps[sid] = set(SPEC_ID_RE.findall(body))
                break
    return ids, deps


def check_spec_partition(tasks, membership, spec_ids, spec_deps):
    """Enforce: every task names a real spec; a phase never mixes specs; every
    spec has tasks; cross-spec task dependencies are declared in the spec graph."""
    errors = []
    for tid in sorted(tasks):
        sid = tasks[tid]["spec"]
        if sid is None:
            errors.append(f"{tid}: missing `Spec` field - every task must name the spec whose ACs it serves")
        elif sid not in spec_ids:
            errors.append(f"{tid}: `Spec: {sid}` names no file under specs/ (known: {', '.join(sorted(spec_ids)) or 'none'})")

    by_phase = {}
    for tid, t in tasks.items():
        ph = membership.get(tid)
        if ph is None or t["spec"] is None:
            continue
        by_phase.setdefault(ph, {}).setdefault(t["spec"], []).append(tid)
    for ph in sorted(by_phase):
        if len(by_phase[ph]) > 1:
            detail = "; ".join(f"{s}: {', '.join(sorted(ts))}" for s, ts in sorted(by_phase[ph].items()))
            errors.append(f"phase {ph} mixes specs ({detail}) - a phase belongs to exactly one spec")

    used = {t["spec"] for t in tasks.values() if t["spec"]}
    for sid in sorted(spec_ids - used):
        errors.append(f"spec '{sid}' has no task in tasks.md - it cannot be implemented or verified")
    ...
    for tid, t in sorted(tasks.items()):
        ...
        for dep in sorted(t["deps"]):
            there = tasks.get(dep, {}).get("spec")
            if not there or there == here:
                continue
            if there not in spec_deps.get(here, set()):
                errors.append(
                    f"{tid} (spec {here}) depends on {dep} (spec {there}) but '{here}' does not "
                    f"declare `Depends on: {there}` - add the spec dependency or move the task"
                )
    return errors


def parse_phase_membership(lines):
    """Map task_id -> phase index, read from the '### Phase N' blocks of the Execution Plan.

    Task bodies live under '## Task Breakdown', not under the phase headers, so
    membership comes from the task IDs each phase block lists (its diagram chain).
    A task is attributed to the FIRST phase that mentions it, so a later phase
    referencing an earlier task cannot steal it.
    """
    membership = {}
    phase_idx = None
    for ln in lines:
        ...
        if pm:
            ...
            continue
        if re.match(r"^#{1,4}\s+\S", stripped) and not pm:
            # Any other heading closes the current phase block. Task Breakdown,
            # Phase Execution Map, etc. must not feed membership.
            if not re.match(r"^#{2,4}\s+Phase\s+\d+", stripped, re.IGNORECASE):
                phase_idx = None
            continue
        if phase_idx is None:
            continue
        for tid in EDGE_RE.findall(stripped.upper()):
            membership.setdefault(tid, phase_idx)
    return membership
...
def check(tasks_path):
    ...
    feature_dir = os.path.dirname(os.path.abspath(tasks_path))
    spec_ids, spec_deps = load_spec_graph(feature_dir)
    multi = spec_ids is not None

    required = REQUIRED_SECTIONS + (REQUIRED_SECTIONS_MULTI if multi else [])
    ...
    if multi:
        errors += check_spec_partition(tasks, membership, spec_ids, spec_deps)
    for tid, t in tasks.items():
